...
class PrimaryFunctionProcessor(
    ...
    override fun processSymbols(resolver: Resolver): List<KSAnnotated> {
        val skyHanniEvent = resolver.getClassDeclarationByName("com.somefrills.events.FrillsEvent")
        ...
        val outputFile = cache.outputFile("com/somefrills/events/core", "GeneratedEventPrimaryFunctionNames")
    ...
    private fun generate(symbols: List<KSClassDeclaration>) {
        ...
        val file = codeGenerator.createNewFile(
            dependencies,
            "com.somefrills.events.core",
            "GeneratedEventPrimaryFunctionNames"
        )
        OutputStreamWriter(file).use {
            it.write("package com.somefrills.events.core\n\n\n")
            it.write("import com.somefrills.events.FrillsEvent\n\n")
            it.write("object GeneratedEventPrimaryFunctionNames {\n")
            it.write("    val map: Map<String, Class<out FrillsEvent>> = mapOf(\n")
            it.write("        $entries\n")
            it.write("    )\n")
            it.write("}\n")
        }
